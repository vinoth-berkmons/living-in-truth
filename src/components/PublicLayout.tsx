...
export const PublicLayout = ({ workspace, children }: PublicLayoutProps) => {
  ...
  const navLinks = [
    { label: t('nav.home', language), to: `/w/${workspace.slug}` },
    { label: t('nav.videos', language), to: `/w/${workspace.slug}/explore?type=video` },
    { label: t('nav.courses', language), to: `/w/${workspace.slug}/courses` },
    { label: t('nav.articles', language), to: `/w/${workspace.slug}/explore?type=article` },
    { label: t('nav.explore', language), to: `/w/${workspace.slug}/explore` },
  ];
      ...
      <header className="sticky top-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="flex h-14 items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-6">
            <Link to={`/w/${workspace.slug}`} className="flex items-center gap-2.5">
              {workspace.themeOverride?.logoUrl ? (
                <img src={workspace.themeOverride.logoUrl} alt={workspace.name} className="h-8 w-8 rounded-lg object-cover" />
              ) : (
              ...
              <span className="hidden font-heading text-base font-bold text-foreground sm:block">{workspace.name}</span>
            </Link>
          ...
          <div className="flex items-center gap-2">
            {/* Search */}
            <Link to={`/w/${workspace.slug}/explore`} className="rounded-lg p-2 text-muted-foreground transition-colors hover:text-foreground">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                ...
                <Link to={`/w/${workspace.slug}/account`} className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground transition-transform hover:scale-110">
                  {user.avatarUrl ? (
              ...
              <Link to={`/w/${workspace.slug}/login`} className="rounded-lg bg-primary px-4 py-1.5 text-sm font-semibold text-primary-foreground transition-all hover:bg-primary/90 hover:scale-105">
        ...
        {mobileMenuOpen && (
          <div className="border-t border-border bg-surface px-4 py-3 lg:hidden animate-fade-in">
            <nav className="flex flex-col gap-1">
              ...
              <Link to={`/w/${workspace.slug}/pricing`} onClick={() => setMobileMenuOpen(false)} className="rounded-md px-3 py-2 text-sm font-medium text-foreground">
          ...
          <div className="grid gap-8 md:grid-cols-4">
            ...
            <div>
              ...
              <nav className="mt-3 flex flex-col gap-2">
                <Link to={`/w/${workspace.slug}/explore`} className="text-sm text-foreground/70 hover:text-foreground">{t('nav.explore', language)}</Link>
                <Link to={`/w/${workspace.slug}/courses`} className="text-sm text-foreground/70 hover:text-foreground">{t('nav.courses', language)}</Link>
              </nav>
            </div>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Account</h4>
              <nav className="mt-3 flex flex-col gap-2">
                <Link to={`/w/${workspace.slug}/pricing`} className="text-sm text-foreground/70 hover:text-foreground">{t('nav.pricing', language)}</Link>
                <Link to={`/w/${workspace.slug}/account`} className="text-sm text-foreground/70 hover:text-foreground">{t('nav.account', language)}</Link>
              </nav>
            </div>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Workspaces</h4>
              <nav className="mt-3 flex flex-col gap-2">
                <Link to="/" className="text-sm text-foreground/70 hover:text-foreground">All Spaces</Link>
              </nav>
            </div>
          </div>
