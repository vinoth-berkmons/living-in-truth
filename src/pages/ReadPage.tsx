...
import { Crown, BookOpen, ArrowLeft, Lock } from 'lucide-react';
            ...
            {contentCats.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {contentCats.map(cat => {
                  ...
                  return <Link key={cat.id} to={`/w/${workspace.slug}/category/${cat.slug}`} className="rounded-md bg-secondary px-2.5 py-1 text-xs font-medium text-secondary-foreground hover:bg-secondary/80">{ctr?.title ?? cat.slug}</Link>;
                })}
              </div>
            )}
              ...
              <div className="relative mt-8">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="rounded-xl border border-border bg-card p-8 text-center shadow-lg">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Subscribe to read the full article</p>
                    <Link to={`/w/${workspace.slug}/pricing`} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
          ...
          <div className="sticky top-20">
            ...
            <div className="space-y-3">
              {related.map(item => {
                ...
                return (
                  <Link key={item.id} to={`/w/${workspace.slug}/read/${item.slug}`} className="group flex gap-3 rounded-lg border border-border/50 bg-card p-2 transition-all hover:border-primary/30 hover:shadow-sm">
